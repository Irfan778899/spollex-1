...
class ReceivablePayableReport:
	...
	def build_data(self):
		...
		for _key, row in self.voucher_balance.items():
			if row.voucher_type != "Sales Invoice":  # Filter out non-"Sales Invoice"
				continue
	...
	def filter_ple_entries_by_user_sales_person(self):
		...
		# Get sales person(s) mapped to this user (via 'user' field in Sales Person)
		sales_persons = frappe.get_all("Sales Person", filters={"custom_user_id": current_user}, pluck="name")
		...
		# Filter ple_entries
		self.ple_entries = [
			ple for ple in self.ple_entries
...
#			options="voucher_type",
		...
		if self.account_type == "Receivable":
			...
			self.add_column(_("Rebate Given"), fieldname="rebate_given")
		else:
			# note: fieldname is still `credit_note`
			self.add_column(_("Debit Note"), fieldname="credit_note")
			self.add_column(_("Rebate Received"), fieldname="rebate_received")
